...
|---|---|---|---|---|---|
| WS-01 | Workspace | Fullscreen neutral canvas with pan/zoom | Pass | `apps/review-workbench/src/styles.css:1213` | Uses full viewport workspace sizing (`.ne-page` + `.ne-workspace`) |
...
| WS-03 | Workspace | Edge panning near viewport borders while dragging | Pass | `apps/review-workbench/src/pages/NodeEditorSandboxPage.tsx:592` | Connect-drag pointer tracking with viewport edge auto-pan loop + `autoPanOnConnect` |
...
| ED-05 | Edit | Internal relations visible as pills in modal with remove action | Pass | `apps/review-workbench/src/pages/NodeEditorSandboxPage.tsx:565` | Node modal lists connected relations and supports per-pill remove action |
