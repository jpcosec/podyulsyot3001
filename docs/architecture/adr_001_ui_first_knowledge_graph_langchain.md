...
1. **Extraction quality**: Flat JSON schemas lose structured information (dates, salary, institution, requirement domains). LLM output drifts between runs.
2. **Fragile identity**: Evidence IDs are positional counters. Adding a profile entry breaks all downstream references.
3. **No cross-referencing**: Experience-Skills, Publication-Project, Education-equivalency-Requirements are implicit. Rediscovered by LLM on every invocation.
...
- Each generation step gets its own instance of this view
...
| Graph visualization | Cytoscape.js (via react-cytoscapejs) | Mature, interactive, supports compound graphs, good layout algorithms, Neo4j integration |
...
The schema is organized into five domains. Every node that originates from a source
document carries a `TextSpan` relationship for View 2 bidirectional highlighting.
...
**Job domain** — everything extracted from a posting. Structured to match the actual
information present in university job board postings (Stellenticket template analysis).
...
Every node in the Job domain that was extracted from text should have at least one
incoming `EXTRACTED_TO` edge from a `TextSpan`. This enables View 2 bidirectional
highlighting. When the user selects text in the source document, the system finds
overlapping `TextSpan` nodes and follows edges to the extracted graph nodes. When
the user clicks a graph node, the system follows edges back to `TextSpan` nodes
and highlights the corresponding ranges in the source document.
...
|-------------|----------|
| Known structured sites | Dedicated adapter + deterministic field extraction |
| Semi-structured sites | Adapter + LLM normalization |
...
- React app scaffold with routing, Cytoscape.js integration, Slate.js integration
...
2. **Hybrid LangChain adoption** (original recommendation from `langchain_langgraph_adoption_evaluation.md`): Lower risk but perpetuates custom maintenance burden and misses tracing/multi-provider benefits.
...
4. **Keep current architecture, fix incrementally**: The flat JSON schema and positional IDs are fundamentally limiting. Incremental fixes would fight the architecture rather than fix it.
