...
from src.translator.providers.google.adapter import GoogleTranslatorAdapter
...
P_FIELDS_TO_TRANSLATE = [
    ...
    "posted_date", "responsibilities", "requirements", "salary",
    "remote_policy", "benefits", "company_description",
    "company_industry", "company_size", "application_deadline",
    "contact_info"
]

def main():
    parser = argparse.ArgumentParser(description="Deterministic Translation Pipeline")
    parser.add_argument("--provider", default="google", choices=list(PROVIDERS.keys()), help="Translation engine to use.")
    parser.add_argument("--source", required=True, help="Job portal source folder to scan (e.g. 'tuberlin', 'stepstone').")
    parser.add_argument("--target_lang", default="en", help="Target language (e.g. 'en').")
    parser.add_argument("--data_dir", default="data/source", help="Base directory for scraped outputs.")
    parser.add_argument("--force", action="store_true", help="Force re-translation even if translated files exist.")
    ...
    for job_dir in os.listdir(source_path):
        ...
        try:
            ...
            if orig_lang == args.target_lang:
                logger.info(f"  [⏭️] Skipping {job_dir} (Already matches target language: '{orig_lang}')")
                # Create exact copies if they are already in the target language
                out_json_path.write_text(json_path.read_text(encoding="utf-8"), encoding="utf-8")
                out_md_path.write_text(md_path.read_text(encoding="utf-8"), encoding="utf-8")
                continue

            logger.info(f"  [🔄] Translating {job_dir} from '{orig_lang}' to '{args.target_lang}' using {args.provider}...")

            # 1. Translate JSON Dictionary Elements
            translated_data = adapter.translate_fields(data, fields=P_FIELDS_TO_TRANSLATE, target_lang=args.target_lang, source_lang=orig_lang)
            translated_data["original_language"] = args.target_lang

            # 2. Translate body markdown
            md_content = md_path.read_text(encoding="utf-8")
            translated_md = adapter.translate_text(md_content, target_lang=args.target_lang, source_lang=orig_lang)
