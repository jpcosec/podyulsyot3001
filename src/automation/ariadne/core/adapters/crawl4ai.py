...
class Crawl4AIAdapter(BrowserAdapter):
    ...
    async def perceive(self) -> SnapshotResult:
        ...
        if not self._crawler:
            raise RuntimeError(
                "Crawl4AIAdapter must be used within an 'async with' block."
            )
    ...
    async def act(self, command: MotorCommand) -> ExecutionResult:
        ...
        if not isinstance(command, CrawlCommand):
            return ExecutionResult(
                status="failed", error=f"Unsupported command: {type(command)}"
            )
        ...
        try:
            ...
            if result.success:
                ...
                return ExecutionResult(
                    status="success",
                    extracted_data=script_result
                    if isinstance(script_result, dict)
                    else {"result": script_result},
                )
    ...
    async def _execute_js(self, script: str) -> ExecutionResult:
        ...
        try:
            ...
            if result.success:
                return ExecutionResult(
                    status="success", extracted_data={"result": result.js_result}
                )
            return ExecutionResult(status="failed", error=result.error_message)
        except Exception as exc:
            return ExecutionResult(status="failed", error=str(exc))
