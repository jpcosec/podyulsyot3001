...
def main(argv: list[str] | None = None) -> int:
    ...
    data_manager.ensure_dir(output_dir.parent)
    data_manager.ensure_dir(output_dir)
    ...
    try:
        ...
        if args.resume:
            thread_id = LangGraphAPIClient.thread_id_for(args.source, args.job_id)
...
def _load_json(path: str, parser: argparse.ArgumentParser) -> Any:
    ...
    try:
        return DataManager().read_json_path(path)
