...
from src.core.ai.generate_documents.storage import DocumentArtifactStore
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.profile:
        try:
            profile_base_data = data_manager.read_json_path(args.profile)
    ...
    try:
        ...
        if state.get("artifact_refs", {}).get("profile_base_data"):
            ...
            if profile_path.exists():
                profile_base = data_manager.read_json_path(profile_path)
            else:
