"""CLI runner for the application pipeline flow."""
...
def main() -> int:
    ...
    _ensure_langsmith_env(verifiable_requested=False)

    parser = argparse.ArgumentParser(description="Run application pipeline flow")
    ...
    _ensure_langsmith_env(verifiable_requested=args.langsmith_verifiable)
...
def _ensure_langsmith_env(verifiable_requested: bool) -> None:
    legacy_key = os.environ.get("LANGSMITH_KEY")
    if not os.environ.get("LANGSMITH_API_KEY") and legacy_key:
        os.environ["LANGSMITH_API_KEY"] = legacy_key

    if not verifiable_requested:
        return
