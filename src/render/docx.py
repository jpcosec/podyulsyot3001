...
class DocumentRenderer:
    ...
    def _make_floating_photo(self, para, photo_path, width=Inches(1.2)):
        ...
        # graphic element lives in the 'a:' drawingml namespace
    ...
    def _bullet_paragraph(self, text: str, font_size=None) -> None:
        ...
        rather than the Wingdings private-use character \\uf0b7, which
        keeps it detectable by ATS keyword/format scanners.
        """
    ...
    def render_header(self, name, titles, contact_info):
        ...
        When a photo is provided it is inserted as a floating anchored image
        (wp:anchor) positioned at the top-right of the margin.  All name /
        title / contact paragraphs remain in the normal paragraph flow, so ATS
        parsers read them without disruption.
        """
        ...
        # Float photo at top-right, attached to the name paragraph
        if self.photo_path:
