...
- Upgraded review-workbench Tailwind integration to v4 Vite-plugin mode in `apps/review-workbench/vite.config.ts` (`@tailwindcss/vite`) and switched stylesheet entry in `apps/review-workbench/src/styles.css` to `@import "tailwindcss"`; removed dependency on PostCSS Tailwind pipeline.
- Completed CV graph redesign gap closure in `apps/review-workbench/src/sandbox/pages/CvGraphEditorPage.tsx`: added deterministic top-level dagre relayout on focus/group state changes, `useUpdateNodeInternals` refresh for entry/skill handles, and loading skeleton rendering before graph data resolves.
- Added CV graph polish + accessibility refinements in `apps/review-workbench/src/styles.css`: focus-visible rings for interactive editor controls, skeleton shimmer components, entry panel intro animation, and improved button/skill focus feedback.
- Captured redesign verification evidence under `.sisyphus/evidence/`: `task-1-tailwind-build.log`, `task-2-mastery-build.log`, `task-3-entry-compact.png`, `task-3-entry-expanded.png`, `task-4-skill-balls.png`, `task-5-group-nodes.png`, `task-7-custom-nodes.png`, `task-8-connect.png`, `task-9-skill-split.png`, `task-11-polish-a11y.png`.
- How to run and verify CV graph redesign slice: run `./scripts/dev-all.sh`, open `http://127.0.0.1:4173/sandbox/cv_graph`, click an entry to expand inline editing, drag an entry handle to a skill handle to create a demonstrates edge, and confirm side panel counters update (`Related skills` and `Demonstrates`).
- Implemented container-node document-structure interactions in `apps/review-workbench/src/sandbox/pages/CvGraphEditorPage.tsx` and `apps/review-workbench/src/sandbox/components/cv-graph/GroupNode.tsx`: drag-stop absorb/extract between free entries and expanded containers, internal vertical reorder persistence, dropzone feedback, and proxy-style dashed edges when relations point to children hidden by collapsed parents.
- Added execution annex to `plan/subplan/deterministic_parity_migration_from_phd.md` with file-level migration matrix, per-WP test gate commands, rollback protocol, commit slicing policy, and step-by-step execution checklist.
- Added container-edit internal-elements controls in the CV graph side panel (`apps/review-workbench/src/sandbox/pages/CvGraphEditorPage.tsx`) with move up/down and extract actions for selected container entries.
- Added architecture documentation from the provided spec blocks in `docs/architecture/cv_graph_container_nodes_uiux_spec.md` and `docs/architecture/cv_graph_container_nodes_frontend_implementation_guide.md`, and linked them in `docs/architecture/README.md` and `docs/index/README.md`.
...
- Updated `/sandbox/node_editor` step slice: moved primary edit interaction to node context (inline `Edit` chip on node hover/selection), kept double-click edit, replaced sidebar node edit action with note, and added `Hide non-neighbors` focus toggle in View Options. To run and verify: `./scripts/dev-all.sh`, open `http://127.0.0.1:4173/sandbox/node_editor`, focus a node, use the node `Edit` chip, and toggle `Hide non-neighbors`.
- Added deterministic layout controls to `/sandbox/node_editor`: `Layout all` (dagre-based full graph arrangement) and `Layout focus` (deterministic radial arrangement of focused node + direct neighbors). Added `dagre` and `@types/dagre` in `apps/review-workbench`. To run and verify: `./scripts/dev-all.sh`, open `http://127.0.0.1:4173/sandbox/node_editor`, click `Layout all`, then focus a node and click `Layout focus`.
- Updated node handle and mode visibility behavior in `/sandbox/node_editor`: switched to four anchor zones per node (`top/right/bottom/left`), enabled `ConnectionMode.Loose` for conceptual mapping, and added clickable in-canvas mode badge (`Mode: ...`) for safe return to browse outside edit states. To run and verify: `./scripts/dev-all.sh`, open `http://127.0.0.1:4173/sandbox/node_editor`, create an edge from top->top handles, then focus a node and click the mode badge to return to browse.
- Added contextual drag-to-connect menu flow in `/sandbox/node_editor`: when a handle drag ends on empty canvas, a floating menu appears at cursor coordinates with options to connect to existing nodes or `+ Create new and connect` (which now opens edit modal for the created node). To run and verify: `./scripts/dev-all.sh`, open `http://127.0.0.1:4173/sandbox/node_editor`, drag from a node handle to empty area, then use the menu actions.
- Expanded typed property editing in `/sandbox/node_editor` node/relation modals: each property now carries explicit data type selection (`string`, `text_markdown`, `number`, `date`, `datetime`, `boolean`, `enum`, `enum_open`) and renders matching input controls. Dynamic attributes now include type assignment at creation/edit time. To run and verify: `./scripts/dev-all.sh`, open `http://127.0.0.1:4173/sandbox/node_editor`, open node edit modal, change a property type (for example to `boolean`), and verify control type switches accordingly.
