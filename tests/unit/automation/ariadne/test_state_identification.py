...
from src.automation.ariadne.models import AriadneState, AriadneMap, AriadneStateDefinition, AriadneObserve, AriadneMapMeta
from src.automation.ariadne.danger_contracts import ApplyDangerReport, ApplyDangerFinding
...
async def test_observe_node_identifies_state():
    ...
    with patch("src.automation.ariadne.repository.MapRepository.get_map", return_value=mock_map):
    ...
    assert "session_memory" not in result or "danger_detected" not in result["session_memory"]
...
async def test_observe_node_detects_danger():
    ...
    # 4. Run Node with Mocks
    with patch("src.automation.ariadne.repository.MapRepository.get_map", side_effect=Exception("No map")):
        with patch("src.automation.ariadne.modes.registry.ModeRegistry.get_mode_for_url") as mock_get_mode:
            mock_mode = MagicMock()
            mock_mode.inspect_danger.return_value = mock_danger_report
