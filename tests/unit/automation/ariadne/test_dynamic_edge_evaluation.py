...
async def test_dynamic_edge_evaluation():
    ...
    mock_state: AriadneState = {
        ...
        "dom_elements": [
            {"selector": "#apply_b", "text": "Apply Now"}
        ],
    ...
    mock_map = AriadneMap(
        ...
        edges=[
            AriadneEdge(from_state="details", to_state="form", intent=AriadneIntent.CLICK, target="apply_a"),
            AriadneEdge(from_state="details", to_state="form", intent=AriadneIntent.CLICK, target="apply_b"),
        ],
        success_states=[],
        failure_states=[]
    )
    ...
    with patch(
        ...
        config = {"configurable": {"executor": mock_executor, "motor_name": "browseros"}}
