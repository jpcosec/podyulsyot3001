...
from src.automation.motors.browseros.cli.models import BrowserOSPlaybook, ExpectedElement
...
def test_run_skips_unresolved_prefill_actions_and_stops_at_dry_run():
    playbook = BrowserOSPlaybook.model_validate_json(
        Path("src/automation/motors/browseros/cli/traces/linkedin_easy_apply_v1.json").read_text(
            encoding="utf-8"
        )
    )
    client = _FakeClient(
