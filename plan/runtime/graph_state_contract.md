...
> Status note (2026-03-20): this document is partially target-state. The current code still carries some transient semantic payloads in `GraphState`, and some examples below use patterns or signatures that no longer match the exact implementation. Treat it as architectural direction, not a line-by-line runtime contract. For actual current behavior, check `src/core/graph/state.py` and `docs/runtime/data_management.md`.
...
- `plan/runtime/graph_reactivity_protocol.md`
