...
Companion to: `plan/step6_13_delta_generation_and_text_reviewers.md`
...
| Choice | Impact |
|---|---|
| Graph-visible node | Must appear in `docs/graph/nodes_summary.md` and `node_io_matrix.md`; gets its own `nodes/generate_documents/` package; checkpoint captures its output |
...
|---|---|---|
| `docs/graph/nodes_summary.md` | Add `generate_documents` node; add `reviewer_<doc>_assist` nodes if graph-visible | Phase B / Phase D |
| `docs/graph/node_io_matrix.md` | Add rows for `generate_documents`, assembler node changes, and assist nodes | Phase B / Phase D |
| `docs/reference/artifact_schemas.md` | Add `DocumentDeltas` schema; add `nodes/<node>/assist/proposed/` path convention | Phase B / Phase D |
| `docs/architecture/core_io_and_provenance_manager.md` | No change needed — already specifies the target; implement against it | — |
...
`nodes/<node>/assist/proposed/state.json` and `nodes/<node>/assist/proposed/view.md` are referenced in Section 7.4 but absent from `docs/reference/artifact_schemas.md`.
