...
> Status note (2026-03-20): this glossary mixes current and target-state artifact descriptions. Some entries below describe planned review, provenance, or finalization conventions that are not uniformly implemented across the current runtime. Use `docs/runtime/data_management.md` for current artifact behavior.
...
- `plan/archive/phd2_stepwise_plan.md`: stepwise implementation and approval plan.
- `plan/archive/index_checklist.md`: compact checklist of all execution steps.

## Architecture docs

- `plan/archive/structure_and_rationale.md`: layer boundaries, design rules, failure taxonomy, and rationale.
- `docs/runtime/graph_flow.md`: graph flow, node contracts, review branches, directives, and invariants.
- `docs/runtime/node_io_matrix.md`: node-by-node input/output contracts and downstream consumers.
- `plan/runtime/graph_reactivity_protocol.md`: distinguishes immediate present-case corrections (emergent evidence) from deferred learning updates (feedback memory).
- `plan/runtime/graph_state_contract.md`: canonical control-plane GraphState ledger and checkpoint identity rules.
- `plan/runtime/artifact_schemas.md`: full JSON schemas for every pipeline artifact.
...
- `plan/runtime/claim_admissibility_and_policy.md`: claim classes, evidence compatibility, coverage transitions, downstream policy.
- `docs/policy/feedback_memory.md`: feedback event schema, storage model, retrieval strategy, conflict model.
- `plan/runtime/sync_json_md.md`: JSON/Markdown sync service API, staleness protection, parser requirements.

## Overview docs

- `plan/archive/project_overview.md`: what the project is and how it works end-to-end.
- `docs/reference/document_glossary.md`: this glossary of document and artifact meanings.

## Prompt docs

- `plan/runtime/templates/prompts/README.md`: prompt pack index and design principles.
- `plan/runtime/templates/prompts/matcher.md` through `plan/runtime/templates/prompts/feedback_distiller.md`: per-prompt role, inputs, outputs, and constraints.
